...
static bool checkAABB(const Transform2D &t1, const Collider &c1,
  ...
  return (t1.x < t2.x + c2.rect.width && t1.x + c1.rect.width > t2.x &&
          t1.y < t2.y + c2.rect.height && t1.y + c1.rect.height > t2.y);
}
...
void CombatSystem::spawnHitbox(Registry &registry, Entity owner) {
  ...
  registry.addComponent<Combat>(hitbox, hitDamage, hitKnockback, owner);
...
void CombatSystem::spawnHitParticles(Registry &registry, float x, float y) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    registry.addComponent<Particle>(p, WHITE, Color{255, 255, 255, 0}, 1.0f,
                                    0.2f);
  }
}
